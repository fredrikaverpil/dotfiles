...
in
{
  ...
  dotfiles.extraServices = {
    ...
    # Dynamic DNS client for Cloudflare - disabled, using custom service below
    # Updates DNS records when public IP changes
    # Secrets are read at runtime from /etc/ddclient/secrets.env
    ddclient = {
      enable = false;  # Disabled - using custom systemd service for environment variable support
    };
  };
  ...
  # Create ddclient user for dynamic DNS service
  users.users.ddclient = {
    isSystemUser = true;
    group = "ddclient";
    description = "ddclient dynamic DNS user";
  };
  ...
  environment.etc = {
    ...
    # ddclient configuration template
    "ddclient/ddclient.conf" = {
      text = ''
        ...
        mail=root                    # Send mail to root on updates
        mail-failure=root            # Send mail on failures
        pid=/var/run/ddclient/ddclient.pid
        ssl=yes                      # Use SSL for API calls
  ...
  systemd.services = {
    ...
    # Override ddclient service to use environment file for secrets
    ddclient = {
      ...
      serviceConfig = {
        ...
        User = "ddclient";
        Group = "ddclient";
        # Only load environment file if it exists
        EnvironmentFile = "-/etc/ddclient/secrets.env";
        ExecStartPre = [
          # Create runtime directory
          "${pkgs.coreutils}/bin/mkdir -p /var/run/ddclient"
          "${pkgs.coreutils}/bin/chown ddclient:ddclient /var/run/ddclient"
          # Check if secrets file exists before proceeding
          "${pkgs.bash}/bin/bash -c 'if [ ! -f /etc/ddclient/secrets.env ]; then echo \"WARNING: /etc/ddclient/secrets.env not found. ddclient will not start until configured.\"; exit 1; fi'"
          # Substitute environment variables in config file
          "${pkgs.envsubst}/bin/envsubst < /etc/ddclient/ddclient.conf > /var/run/ddclient/ddclient.conf"
          "${pkgs.coreutils}/bin/chown ddclient:ddclient /var/run/ddclient/ddclient.conf"
          "${pkgs.coreutils}/bin/chmod 600 /var/run/ddclient/ddclient.conf"
        ];
        ExecStart = "${pkgs.ddclient}/bin/ddclient -daemon 300 -file /var/run/ddclient/ddclient.conf";
        ...
        ProtectHome = true;
        ReadWritePaths = [ "/var/run/ddclient" ];
      };
      # Don't start automatically - user must configure secrets first
      wantedBy = [ ];
    };
  };
  ...
  dotfiles.extraSystemPackages = with pkgs; [
    ...
    # Dynamic DNS tools
    ddclient        # Dynamic DNS client for Cloudflare
