...
The VM has 3.9 GB. `lua-language-server` peaked at **2 GB** and the kernel OOM
killer took it three times in four minutes on 2026-09-06, each kill marking the
enclosing `app-ghostty-surface-transient-*.scope` failed. It reads as "Neovim
crashed", and it is not: the killed process in every `Out of memory:` line is
`lua-language-se`, never `nvim`. Nothing lands in `coredumpctl` — an OOM kill
leaves no core — so the crash looks sourceless from inside the editor.

The trigger is the LuaLS root. Opening any Lua buffer under `~/.dotfiles` roots
it at the repo's `.git`, and `nvim-fredrik/after/lsp/lua_ls.lua` then loads that
workspace *plus* `workspace.library` — VIMRUNTIME and every installed plugin.
That is comfortable on the Mac and does not fit here. `hypr/.luarc.json` fixes
it for the config files that matter by being a root marker itself: LuaLS roots
at `~/.config/hypr` — from either the `~/.config` path or the `stow/` one,
since `root_dir` walks up from the buffer, not from `cwd` — indexes three files
and the Hyprland stub, and stays small. The cost is a **second lua_ls process**
whenever a `~/.dotfiles`-rooted one is already running; `reuse_client` keys on
`root_dir`, and two roots mean two clients.

Nothing else here has that protection. Editing the shell's Lua from inside the
VM will hit the same ceiling.

**A `.luarc.json` replaces a client setting rather than merging with it**, so
the repo-root one has to repeat the whole `workspace.ignoreDir` list that
`nvim-fredrik/after/lsp/lua_ls.lua` sets — that list prunes the library roots
too, and a root file naming only `stow` silently un-prunes every plugin's
`tests/`, `spec/` and `node_modules/`. Whether restoring it alone keeps LuaLS
...
**Hyprland ships LuaLS types for the whole `hl` API**, generated by its
`scripts/generateLuaStubs.py`, at
`/run/current-system/sw/share/hypr/stubs/hl.meta.lua` (1777 lines; it declares
`hl` as a global, so nothing else is needed for completion). The stowed
`hypr/.luarc.json` points `workspace.library` at that directory, which also
makes `~/.config/hypr` its own small LuaLS root — see "lua_ls OOMs the VM"
for why that second effect matters more than the first.
...
- **The Lua option names are underscored; `hyprctl`'s are not.**
  ...
  Take the spelling from the stub below, never from a `hyprctl` option path.
...
menu's Style › Wallpaper) opens the picker; `qs ipc call wallpaper` also takes
