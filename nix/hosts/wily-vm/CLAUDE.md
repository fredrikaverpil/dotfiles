...
Google and Proton calendars, read in the browser, with reminders as desktop
notifications. The stack is GOA (accounts and OAuth tokens) -> EDS
(`evolution-data-server`: sync, and the `evolution-alarm-notify` daemon) ->
`org.freedesktop.Notifications`, which Quickshell already owns. GNOME Calendar
is only a viewer; Evolution is kept for two settings nothing else exposes.

- **Add the Google account in `gnome-online-accounts-gtk`, never in
  Evolution.** nixpkgs builds EDS without `-DWITH_GOOGLE_CLIENT_ID` -- grepping
  its libraries for `apps.googleusercontent.com` returns nothing -- so
  Evolution's own Google account type has no OAuth client and hangs forever at
  *"Requesting access token, please wait..."* with `URL: none-local://`. The
  same grep against `gnome-online-accounts` returns GNOME's registered client
  id, so GOA is the only working path. Both Evolution routes fail identically;
  "Collection Account" is no better than "Google".
- **`goa-daemon` does not re-read `~/.config/goa-1.0/accounts.conf`.** After the
  account is written, the D-Bus tree still holds only `/Manager` and no
  `/Accounts/...`, so EDS sees nothing and the UI looks like it failed. Restart
  it (`pkill -f goa-daemon`, it is D-Bus activated), then
  `systemctl --user restart evolution-source-registry` -- the GOA-backed
  `.source` files appear in `~/.config/evolution/sources/` only after that.
  `busctl --user call org.gnome.OnlineAccounts /org/gnome/OnlineAccounts/Accounts/<id>
  org.gnome.OnlineAccounts.OAuth2Based GetAccessToken` is the readback: a token
  means done, *"No credentials found in the keyring"* means the step below.
- **The keyring is what actually blocks the OAuth flow.** GOA stores the refresh
  token through libsecret, synchronously. `services.gnome.gnome-keyring.enable`
  wires `pam_gnome_keyring` into `/etc/pam.d/login`, but **PAM only runs at
  login**: a session that predates the rebuild enabling it never got the keyring
  unlocked, and the store falls through to a `gcr-prompter` dialog. Check
  `loginctl show-session <id> -p Timestamp` against the rebuild before debugging
  anything else -- exiting and restarting Hyprland is not a new login, the tty
  must be logged out. Keep the keyring password identical to the login password
  or PAM cannot unlock it and the prompt returns every session.
- **Hyprland's ANR dialog stacks on top of the gcr prompt**, which makes a
  perfectly normal blocking prompt look like a crashed app -- "Online Accounts
  is not responding", Terminate/Wait -- while the password dialog sits behind
  it. Click Wait. `misc:enable_anr_dialog` turns it off if it keeps getting in
  the way.
- **Do not give `evolution-alarm-notify.service` an `ExecStart` or
  `Restart=always`.** EDS ships the unit (`Type=dbus`,
  `WantedBy=graphical-session.target`); a second `ExecStart` makes systemd
  refuse it outright (*"more than one ExecStart= setting"*), and `always` turns
  it into a five-second spin loop -- it is a GApplication holding
  `org.gnome.Evolution-alarm-notify`, so any second instance sees the name taken
  and exits **0**, which `always` reads as a reason to retry. Only `wantedBy` is
  set here. Note it can also be running inside Evolution's own app scope, which
  is why the name can be owned while the unit reads inactive.
- **Reminders arrive at urgency 1 and are promoted to critical here.** Off the
  wire: `app_name "evolution-alarm-notify"`, `desktop-entry
  "org.gnome.Evolution-alarm-notify"`, `urgency` byte 1, `expire_timeout -1`.
  EDS never calls `g_notification_set_priority` (absent from its symbols), so
  without promotion a reminder is an 8s toast -- and per the notification
  section above, a normal toast that arrives while the session is locked expires
  unseen behind the lock surface. `criticalSenders` in
  `plugins/notifications/NotificationLogic.js` forces it to critical, matched on
  `desktopEntry` with `appName` as fallback, which also makes it survive to
  unlock and break through DND. Those Evolution reminders additionally play
  `alarm-clock-elapsed.oga` through `pw-play`; other critical notifications do
  not. The call sits after the DND check, so DND still silences what it
  suppresses.
- **GNOME Calendar has only one relevant desktop setting:**
  `org.gnome.desktop.calendar week-start-day` is set to `monday` declaratively
  through Home Manager's `dconf.settings` in `users/fredrik.nix`. Calendar 50
  reads it through the Settings portal at startup, without changing `LC_TIME`
  and its date/time-format side effects. Anything else configurable lives in
  Evolution: `Edit > Preferences > Calendar and Tasks > Reminders` holds the
  per-calendar *default reminder*, which is the fallback for feeds whose events
  carry no `VALARM`, and Evolution is also the only view for invitations still
  at `PARTSTAT=NEEDS-ACTION`. Proton's read-only links are added there too:
  `File > New > Calendar > On The Web`.
- **The alternatives were priced and rejected.** `thunderbird` (153.0.3) has
  `aarch64-linux` in `meta.platforms` but 404s on cache.nixos.org, so it is a
  source build on a VM with no room for one -- and it would be a resident GUI
  used as a reminder daemon. `kdePackages.korganizer` pulls `mariadb-server`
  into its closure via Akonadi. Browser tabs were rejected earlier: Google
  Calendar's web notifications are in-page, so they need the tab open forever
  and stop silently when it is not.

- **`wily-calendar-watchdog` makes the silent dependencies observable.** Its
  timer runs two minutes into a graphical session and on each wall-clock
  quarter hour. `Persistent=true` only applies to the `OnCalendar` trigger --
  not a monotonic `OnUnitActiveSec` trigger -- so an elapsed quarter-hour runs
  on resume instead of being lost. The check uses the notifier's D-Bus name
  (the unit can read inactive when Evolution owns it), the newest non-trash
  `cache.db` mtime (two-hour threshold), and `GetAccessToken` on each Google
  GOA account found through the manager tree. Each current fault is a critical
  `notify-send` once, recorded in
  `~/.local/state/wily-calendar-watchdog-faults`; its recovery produces one
  ordinary 8s toast. A found fault is a successful check, while a broken script
  (including a D-Bus call stuck for its one-minute start timeout) triggers the
  independent `wily-calendar-watchdog-failed` critical alert via `OnFailure`.

Two checks remain outstanding (whether the sound is audible, and whether the
`desktopEntry` branch of `criticalSenders` ever matches). Google's exported ICS
is also served from cache, so a same-morning reschedule can lag; GOA/CalDAV
avoids that for Google but the Proton links are plain ICS. The
`appointment-soon` icon does not resolve (`WARN: Could not load icon
"appointment-soon"`), so reminder toasts are iconless until an icon theme is
installed.
