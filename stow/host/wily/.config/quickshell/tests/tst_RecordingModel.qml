...
TestCase {
  ...
  function test_camera_command() {
    ...
    // A share of the captured frame's short side, whatever that frame is.
    verify(Recording.cameraCommand("/dev/video0", 1, { width: 800, height: 480 })
      .indexOf("--autofit=120x120") > 0)
    verify(Recording.cameraCommand("/dev/video0", 1, { width: 2560, height: 1440 })
      .indexOf("--autofit=360x360") > 0)
    verify(Recording.cameraCommand("/dev/video0", 1, { width: 2560, height: 1440 }, 0.15)
      .indexOf("--autofit=216x216") > 0)
  }
  ...
  function test_camera_position(data) {
    ...
    compare(Recording.cameraPosition(data.region, screen, 32, data.fraction), data.want)
  }

  // A larger share leaves the same margin and starts further up and left.
  function test_camera_position_follows_the_size() {
    const region = { x: 0, y: 200, width: 800, height: 800 }
    const screen = Recording.screenAt(screens, region)
    compare(Recording.cameraPosition(region, screen, 32, 0.15),
      { x: 800 - 120 - 32, y: 200 - 32 + 800 - 120 - 32 })
    compare(Recording.cameraPosition(region, screen, 32, 0.35),
      { x: 800 - 280 - 32, y: 200 - 32 + 800 - 280 - 32 })
  }
