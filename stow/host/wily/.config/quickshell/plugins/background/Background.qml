...
Scope {
  ...
  function open(slot) { picker.open(slot || "workspace") }
  function close() { picker.close() }
  function toggle(slot) { picker.shown ? picker.close() : picker.open(slot || "workspace") }
  ...
  IpcHandler {
    ...
    function toggle(): void { background.toggle("workspace") }
    function open(): void { picker.open("workspace") }
    function openBackdrop(): void { picker.open("backdrop") }
  ...
  component Wall: PanelWindow {
    ...
    Item {
      ...
      function swap() {
        ...
        if (String(over.source) === url) return
        fade.enabled = false
        over.opacity = 0
        fade.enabled = true
