...
TestCase {
  ...
  function test_events_split_into_days_with_all_day_first() {
    ...
    const week = { uid: "week", start: "2026-09-14T00:00:00Z", tag: "G", summary: "Week 38", location: "",
      ...
      until: new Date(2026, 8, 16).getTime(), time: "all day" }
    const night = (time) => ({ uid: "night", start: local(15, 23), tag: "", summary: "(no title)", location: "",
      ...
      until: new Date(2026, 8, 16, 1).getTime(), time: time })
    compare(days.map(day => day.events), [
      [week, { uid: "late", start: local(14, 17), tag: "❤️", summary: "Late", location: "Room",
        ...
        at: new Date(2026, 8, 14, 17).getTime(), until: new Date(2026, 8, 14, 17, 30).getTime(),
        time: "17:00–17:30" }],
      [week, night("23:00–…")],
      [night("…–01:00")],
    ])
  ...
  function test_unreadable_dates_show_once_on_the_first_day() {
    ...
    const unreadable = (uid, summary, start, tag) => ({ uid: uid, start: start, tag: tag, summary: summary,
      location: "", meetingUrl: "", allDay: false, at: 0, until: 0, time: "?" })
    compare(days.map(day => day.events), [
      [
        ...
        { uid: "timed", start: local(14, 9), tag: "", summary: "Timed", location: "", meetingUrl: "",
          ...
          until: new Date(2026, 8, 14, 10).getTime(), time: "09:00–10:00" },
      ],
      [],
      [],
    ])
