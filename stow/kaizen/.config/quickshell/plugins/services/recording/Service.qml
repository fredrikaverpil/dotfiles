...
Item {
  ...
  readonly property string statePath: Ui.Paths.state + "/recording.json"
  ...
  function grab(rect) {
    snap(file => ["grim", "-g", Model.formatGrimRegion(rect), file])
  }

  // Screenshots the focused "screen" or "window".
  function shoot(mode) {
    snap(file => Ui.Compositor.screenshot(mode, file))
  }
  ...
  function snap(command) {
    ...
    Quickshell.execDetached(["sh", "-c",
      ...
      + '[ -z "$action" ] || exec satty --filename "$file" --output-filename "$file" --early-exit '
      + '--actions-on-enter save-to-clipboard --save-after-copy --copy-command wl-copy',
      "sh", file].concat(command(file)))
  }
